#!/bin/bash
! [ $# -eq 1 ] && echo "Usage: ./buildprd.sh remotegit" && exit 1
...
echo $JARFILE
mvn deploy:deploy-file -DgroupId=$GROUPID -DartifactId=$ARTIFACTID -Dversion=$VERSION -Dpackaging=jar -Dfile=$JARFILE -Durl=https://hkgabpwapp24.aia.biz:9443/repository/ruleengine-releases/ -DrepositoryId=nexus-releases -Dmaven.wagon.http.ssl.insecure=true -Dmaven.wagon.http.ssl.allowall=true
